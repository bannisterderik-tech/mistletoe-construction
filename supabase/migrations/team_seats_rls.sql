...
select 'role fn' as check, public.my_role() as value
union all
select 'team_seats table', (select count(*)::text from public.team_seats)
union all
select 'policies added', (select count(*)::text from pg_policies
  where schemaname='public' and policyname like any (array['%_sales_%','%_field_%','team_seats_%']));
